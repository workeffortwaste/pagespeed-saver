...
(() => {
  ...
  const injectHTML = (device) => {
    ...
    const callback = async (mutationsList, observer) => {
      ...
      await sleep(1000);
      // Insert button
      [...document.querySelectorAll(`*[aria-labelledby="${device}_tab"] button#url_tab`)].pop().insertAdjacentHTML('afterend', escapeHTMLPolicy.createHTML(html))
